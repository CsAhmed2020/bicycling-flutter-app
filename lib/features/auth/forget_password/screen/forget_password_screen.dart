...
class _ForgetPasswordScreenWithBloc
    ...
    return BlocConsumer<ForgetPasswordBloc, ForgetPasswordState>(
      listener: (ctx, state) async {
        ...
        if (state is ForgetPasswordErrorState) {
          ...
          showFeedbackMessage("");
          _openOTPCodeScreen("state.forgetPasswordSuccessfulResponse.uuid", "state.forgetPasswordSuccessfulResponse.resetToken");
        }
      },
      builder: (ctx, state) {
      ...
      children: [
        Text(translate(LocalizationKeys.forgotPassword)!,
            style: textTheme.labelLarge?.copyWith(
              fontSize: 24.spMin,
              color: AppColors.appButtonBlueText,
            )),
  ...
  void _openOTPCodeScreen(String uuid, String restToken) {
    OtpScreen.open(context, uuid, restToken: restToken, openAfterCheckOtp: OpenAfterCheckOtp.restPasswordScreen);
  }
}
